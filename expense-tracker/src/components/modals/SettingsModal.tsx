    ...
    <div className={`modal-overlay ${isClosing ? 'closing' : ''}`} onClick={(e) => { if (e.target === e.currentTarget) handleClose(); }}>
      <div className={`modal-content settings-modal-content ${isClosing ? 'closing' : ''}`} style={{ maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        {activeTab === 'wallets' ? (
          <>
            {/* Total summary only */}
            <div style={{ background: 'linear-gradient(135deg, var(--card-bg-2) 0%, var(--card-bg-3) 100%)', borderRadius: '20px', padding: '20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {([['В Євро', totalEur, 'EUR'], ['В Доларах', totalUsd, 'USD'], ['В Гривні', totalUah, 'UAH']] as const).map(([label, value, curr]) => (
                  <div key={curr} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: '18px', fontWeight: 700, color: 'var(--text-primary)' }}>
                      <AnimatedNumber value={value} formatter={(v) => `${v.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${CURRENCY_SYMBOLS[curr]}`} />
                    </span>
                  </div>
                ))}
              </div>
