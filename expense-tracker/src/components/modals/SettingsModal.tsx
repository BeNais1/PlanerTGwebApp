  ...
  return (
    <div className={`modal-overlay ${isClosing ? 'closing' : ''}`} onClick={(e) => { if (e.target === e.currentTarget) handleClose(); }}>
      <div className={`modal-content settings-modal-content ${isClosing ? 'closing' : ''}`} style={{ maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        {/* Tabs */}
        <div style={{ display: 'flex', gap: '4px', background: 'var(--apple-surface-2)', borderRadius: '12px', padding: '3px' }}>
          {(['wallets', 'categories'] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              style={{
                flex: 1, padding: '8px', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: 600, cursor: 'pointer',
                background: activeTab === tab ? 'var(--apple-surface-3)' : 'transparent',
        ...
        {activeTab === 'wallets' ? (
          <>
            ...
            <div className="modal-input-group" style={{ flexShrink: 0, marginTop: '10px' }}>
                        ...
                        {editingWallet === c ? (
                          <div style={{ display: 'flex', gap: '8px', flex: 1, marginRight: '10px' }}>
                            <input type="text" value={editingName} onChange={e => setEditingName(e.target.value)}
                              style={{ flex: 1, padding: '4px 8px', borderRadius: '4px', border: 'none', background: 'var(--apple-surface-3)', color: 'white' }} autoFocus />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {categories.map(cat => (
                <div key={cat.id} style={{
                  ...
                  background: 'var(--apple-surface-2)', borderRadius: '14px',
                }}>
              ...
              <>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  {hiddenDefaults.map(cat => (
                    <div key={cat.id} style={{
                      display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 14px',
                      background: 'var(--apple-surface-2)', borderRadius: '14px', opacity: 0.6,
                    }}>
                      <span style={{ fontSize: '20px' }}>{cat.icon}</span>
                      <span style={{ flex: 1, fontSize: '15px', fontWeight: 500, color: 'white' }}>{cat.name}</span>
                ...
                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                  {['#FF9500', '#007AFF', '#34C759', '#AF52DE', '#FF2D55', '#FF3B30', '#5856D6', '#5AC8FA', '#A2845E', '#8E8E93'].map(color => (
                    <button key={color} onClick={() => setNewCatColor(color)}
                      style={{
                        width: '32px', height: '32px', borderRadius: '50%', background: color, border: newCatColor === color ? '3px solid white' : '3px solid transparent',
